...
Given('my character is at level {int}', async function ({ page }: { page: Page }, level: number) {
  await page.goto('/creator');
  await page.getByTestId('class-card-warrior').click();
  await page.getByTestId('race-card-human').click();
  await page.getByTestId('tab-stats').click();
...
When('I view my character sheet', async function ({ page }: { page: Page }) {
  await page.getByTestId('adventure-tab-character').click();
  await page.waitForTimeout(300);
});

Then('I should see my level displayed as {int}', async function ({ page }: { page: Page }, level: number) {
...
Given('my character has {int} XP', async function ({ page }: { page: Page }, xp: number) {
...
When('I gain {int} XP', async function ({ page }: { page: Page }, xp: number) {
  await page.evaluate((xpVal) => {
    const key = 'rpg_characters';
    const list = JSON.parse(localStorage.getItem(key) || '[]');
    if (list.length > 0) {
...
Then('I should see my XP progress bar updated', async function ({ page }: { page: Page }) {
  const xpBar = page.getByTestId('xp-bar');
  await expect(xpBar).toBeVisible();
});

Then(/I should see "([^"]*)" displayed/, async function ({ page }: { page: Page }, text: string) {
  await expect(page.getByText(text)).toBeVisible();
});

// Scenario: XP threshold triggers level up
Then('a level up modal should appear', async function ({ page }: { page: Page }) {
  const modal = page.getByTestId('level-up-modal');
  await expect(modal).toBeVisible();
});

Then(/the modal should display "([^"]*)"/, async function ({ page }: { page: Page }, text: string) {
  const modal = page.getByTestId('level-up-modal');
  await expect(modal).toContainText(text);
});

Then('the modal should show remaining stat points to allocate', async function ({ page }: { page: Page }) {
  const modal = page.getByTestId('level-up-modal');
  await expect(modal).toBeVisible();
});

// Scenario: Allocate stat point on level up
Given('I have just leveled up to level {int}', async function ({ page }: { page: Page }, level: number) {
...
When('I allocate a stat point to {string}', async function ({ page }: { page: Page }, statName: string) {
...
Then('the stat point allocation should be recorded', async function ({ page }: { page: Page }) {
  // Verify the modal is closed and stats are updated
  const modal = page.getByTestId('level-up-modal');
  await expect(modal).not.toBeVisible();
});

Then('remaining stat points should decrease by {int}', async function ({ page }: { page: Page }, amount: number) {
  // The stat points should be reflected in the character sheet
  const levelBadge = page.locator('.sheet-level-badge .level-number');
  await expect(levelBadge).toBeVisible();
});

Given('I have allocated a stat point to {string}', async function ({ page }: { page: Page }, statName: string) {
...
Then('other stats should remain unchanged', async function ({ page }: { page: Page }) {
...
Then('my max HP should increase', async function ({ page }: { page: Page }) {
...
Then('the HP increase should reflect the class hitDie and CON modifier', async function ({ page }: { page: Page }) {
...
Then('my max MP should increase', async function ({ page }: { page: Page }) {
...
Then('the MP increase should reflect INT, WIS, and level scaling', async function ({ page }: { page: Page }) {
