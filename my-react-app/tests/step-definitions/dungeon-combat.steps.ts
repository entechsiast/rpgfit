...
When(/I enter the "([^"]*)" dungeon/, async function (name: string) {
  const page = this.page as Page;
...
Then('the combat log should show battle results', async function () {
  const page = this.page as Page;
  const combatLog = page.getByTestId('combat-log');
  await expect(combatLog).toBeVisible();
});

When('combat resolves', async function () {
  const page = this.page as Page;
  // The combat simulator auto-resolves with a timer; wait for it
  await page.waitForTimeout(5000);
});

Then('I should see the dungeon complete results', async function () {
  const page = this.page as Page;
  const results = page.getByTestId('combat-results');
  await expect(results).toBeVisible();
});

Then(/I should see (.*?) and (.*?) rewards/, async function (xp: string, gold: string) {
  const page = this.page as Page;
...
When('I switch to the character tab', async function () {
  const page = this.page as Page;
  await page.getByTestId('adventure-tab-character').click();
  await page.waitForTimeout(300);
});

Then('I should see my character name', async function () {
  const page = this.page as Page;
  const name = page.locator('.sheet-info h3');
  await expect(name).toBeVisible();
});

Then('I should see my level', async function () {
  const page = this.page as Page;
  const levelBadge = page.locator('.sheet-level-badge');
  await expect(levelBadge).toBeVisible();
});

Then('I should see all six core stats', async function () {
  const page = this.page as Page;
...
Then('I should see my HP and MP values', async function () {
  const page = this.page as Page;
...
When('I switch to the inventory tab', async function () {
  const page = this.page as Page;
  await page.getByTestId('adventure-tab-inventory').click();
  await page.waitForTimeout(300);
});

Then('I should see the equipment section', async function () {
  const page = this.page as Page;
...
Given('I have at least {int} gold', async function (amount: number) {
  const page = this.page as Page;
...
Then('I should see the shop items', async function () {
  const page = this.page as Page;
...
When(/I buy a "([^"]*)"/, async function (itemName: string) {
  const page = this.page as Page;
...
Then('my gold should decrease by {int}', async function (amount: number) {
  const page = this.page as Page;
  const goldDisplay = page.getByTestId('gold-display');
  await expect(goldDisplay).toBeVisible();
});

Then(/I should have one "([^"]*)" in my inventory/, async function (itemName: string) {
  const page = this.page as Page;
...
Given('I have a "([^"]*)" in my inventory', async function (itemName: string) {
  const page = this.page as Page;
...
Then('my current HP is less than my max HP', async function () {
  const page = this.page as Page;
  // This is a precondition check; in practice the HP display would show this
});

Then(/I should see a "([^"]*)" button for the (.*)/, async function (btnText: string, itemName: string) {
  const page = this.page as Page;
  const btn = page.getByTestId(`btn-use-${itemName.toLowerCase().replace(/\s+/g, '_')}`);
  await expect(btn).toHaveText(btnText);
});

When(/I use the "([^"]*)"/, async function (itemName: string) {
  const page = this.page as Page;
  const itemId = itemName.toLowerCase().replace(/\s+/g, '_');
...
Then('my HP should increase', async function () {
  const page = this.page as Page;
...
When('I click the "Edit Character" link', async function () {
  const page = this.page as Page;
  const link = page.getByText(/\uD83D\uDD8A\uFE0F Edit Character/);
  await link.click();
});

Then('I should be on the character creator page', async function () {
  const page = this.page as Page;
  await expect(page).toHaveURL('/creator');
});

// Additional dungeon exploration steps
Given('I am on the dungeons tab', async function () {
  const page = this.page as Page;
  await page.getByTestId('tab-dungeons').click();
  await page.waitForTimeout(300);
});

When('dungeons are displayed', async function () {
  const page = this.page as Page;
...
Then(/"([^"]*)" should appear locked/, async function (name: string) {
  const page = this.page as Page;
  const dungeonId = name.toLowerCase().replace(/\s+/g, '_');
  const dungeonCard = page.getByTestId(`dungeon-card-${dungeonId}`);
...
Then('locked dungeons should display a level requirement message', async function () {
  const page = this.page as Page;
  const lockedMsg = page.locator('.dungeon-locked-msg');
  await expect(lockedMsg).toBeVisible();
});

When(/I click on "([^"]*)" dungeon card/, async function (name: string) {
  const page = this.page as Page;
  const dungeonId = name.toLowerCase().replace(/\s+/g, '_');
  const dungeonCard = page.getByTestId(`dungeon-card-${dungeonId}`);
  await dungeonCard.click();
...
Then(/I should see the dungeon name "([^"]*)"/, async function (name: string) {
  const page = this.page as Page;
  await expect(page.getByText(name)).toBeVisible();
});

Then(/I should see the difficulty "([^"]*)"/, async function (difficulty: string) {
  const page = this.page as Page;
  await expect(page.getByText(difficulty)).toBeVisible();
});

Then('I should see a list of monsters', async function () {
  const page = this.page as Page;
  const monsterList = page.locator('.monster-list');
  await expect(monsterList).toBeVisible();
});

Then(/I should see a boss named "([^"]*)"/, async function (name: string) {
  const page = this.page as Page;
  await expect(page.getByText(name)).toBeVisible();
});

Then('I should see completion rewards', async function () {
  const page = this.page as Page;
  const rewards = page.locator('.reward-list');
  await expect(rewards).toBeVisible();
});

Then(/"([^"]*)" should display a completion badge/, async function (name: string) {
  const page = this.page as Page;
  const dungeonId = name.toLowerCase().replace(/\s+/g, '_');
  const dungeonCard = page.getByTestId(`dungeon-card-${dungeonId}`);
...
Then('the completion badge should contain a checkmark symbol', async function () {
  const page = this.page as Page;
...
Given('I have completed {string}', async function (dungeonName: string) {
  const page = this.page as Page;
...
Given('I have not selected a class or race', async function () {
  const page = this.page as Page;
...
When('I navigate to the dungeons tab', async function () {
  const page = this.page as Page;
  await page.goto('/creator');
  await page.waitForTimeout(500);
  await page.getByTestId('tab-dungeons').click();
...
Then('I should see a placeholder message', async function () {
  const page = this.page as Page;
  const placeholder = page.locator('.dungeons-placeholder');
  await expect(placeholder).toBeVisible();
});

Then(/the placeholder should say "([^"]*)"/, async function (text: string) {
  const page = this.page as Page;
  await expect(page.getByText(text)).toBeVisible();
});

Then('no dungeon cards should be visible', async function () {
  const page = this.page as Page;
  const dungeonCards = page.locator('.dungeon-card');
  await expect(dungeonCards).toHaveCount(0);
});

Then('dungeons should appear in order of increasing difficulty', async function () {
  const page = this.page as Page;
...
Then(/"([^"]*)" should appear before "([^"]*)"/, async function (first: string, second: string) {
  const page = this.page as Page;
