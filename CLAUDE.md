...
2. **Close completed issues — ONLY after linked PR is merged** — Never close an issue before its linked PR is merged to `main`. If the PR is still open (even if CI passes), leave the issue open.

   **Verification before closing:**
   ```bash
   gh pr list --repo entechsiast/rpgfit --state merged --json number,title | Select-String "<issue-number>"
   ```
   Only close the issue if the PR appears in the merged list. If the PR is still open, set the project board status to "In Progress" (not Done) and wait for merge.
