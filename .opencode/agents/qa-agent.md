...
## Bug Issue Creation Workflow

When you discover a bug during testing (test failures, unexpected behavior, missing functionality), create a detailed GitHub issue so the Engineering Lead can triage and assign it.

### Step 1 — Check for Existing Duplicate Issues

**Always check for existing bug reports before creating a new issue.**

```bash
# Search existing issues for similar bug reports
gh issue list --repo entechsiast/rpgfit --state all --label bug --json title,number,labels

# Or search by keyword
gh issue list --repo entechsiast/rpgfit --state all --search "<keyword from bug description>"
```

**If a matching issue already exists:**
- Do NOT create a duplicate
- Comment on the existing issue with your findings
- Link the existing issue to your current work issue

**If no matching issue exists:** proceed to Step 2.

### Step 2 — Determine Sprint Assignment

Check the current active sprint from the project board before creating the issue:

```bash
gh project item-list 2 --owner entechsiast --format json --limit 50
```

Identify which sprint has active/in-progress items. Use that sprint for the new issue.
...
- `bug: stat allocation fails when all points exhausted`
- `bug: equipment slot not updating after item equip`
- `bug: combat damage formula returns negative values`

Create the issue with a structured bug report body:
...
### Step 4 — Bug Report Body Template

Use this structured template for every bug issue:

````
## Bug Report

**Scenario that failed:**
[Describe the test scenario or user flow where the bug was discovered]
...
- Character state: [class, race, level, etc.]
- Test mode: [headed / headless / CI]
...
**Related issue:** #[number] (if linked to a test or feature issue)
````
...
| Label | When to use |
|-------|-------------|
| `bug` | Always — this is the primary label |
| `test-failure` | When the bug was discovered via a failing test |
| `high-priority` | When the bug blocks core functionality |
| `ui` | When the bug is visual / layout related |
| `data` | When the bug is in game data (stats, skills, equipment) |
| `regression` | When a previously working feature has broken |

```bash
gh issue edit <issue-number> --repo entechsiast/rpgfit --add-label "bug,test-failure"
```
...
```bash
# For Sprint 2 (use the correct sprint ID for the active sprint)
gh project item-field 2 --owner entechsiast --url https://github.com/entechsiast/rpgfit/issues/<issue-number> --field-id PVTSSF_lAHOAWZJdM4BXz_qzhS-TPY --value abfbfde7
```
...
### Step 7 — Report to Engineering Lead

After creating the issue, hand off to the Engineering Lead:

1. Comment on your current work issue with the bug issue number and summary
2. Use the task tool to notify the Engineering Lead:

```
task: "Bug discovered during QA — issue <bug-issue-number> created"
prompt: "During QA testing of <current-issue>, I discovered a bug. Created issue #[bug-issue-number]: <bug title>. Details: <brief summary>. Please triage and assign to the appropriate sprint."
```
...
|----------|----------|---------|
| **high** | Blocks testing, crashes app, data loss, core feature broken | Combat simulator crashes on level-up |
| **medium** | Feature works but produces incorrect results | Damage formula returns wrong value |
| **low** | Cosmetic, UX polish, edge case with workaround | Misaligned button, typo in tooltip |
